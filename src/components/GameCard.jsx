...
export function GameCard({ game, onEdit, onDelete, onStatusChange }) {
  ...
  return (
    ...
    >
      {/* Capa */}
      <div style={{ width:80, flexShrink:0, background:'var(--bg3)' }}>
        {game.coverUrl
          ? <img src={game.coverUrl} alt={game.title} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />
          : <div style={{ width:'100%', height:'100%', minHeight:110, display:'flex', alignItems:'center', justifyContent:'center', fontSize:30 }}>🎮</div>
        }
      </div>
      ...
      <div style={{ flex:1, padding:'14px 16px', minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:6 }}>
          ...
          {avg() && (
            <div style={{ textAlign:'center', flexShrink:0 }}>
              <div style={{ fontSize:20, fontWeight:700, color:'var(--amber)', fontFamily:'var(--font-display)', lineHeight:1 }}>{avg()}</div>
        ...
        {/* Obs */}
        {game.obs && (
          <p style={{ fontSize:13, color:'var(--text2)', lineHeight:1.55, marginBottom:8, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' }}>
            "{game.obs}"
          </p>
        )}
...
function ActionBtn({ children, onClick, accent, danger }) {
  return (
    <button onClick={onClick} style={{
      background: accent ? 'var(--accent-bg)' : danger ? 'var(--red-bg)' : 'var(--bg3)',
